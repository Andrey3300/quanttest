...
class CandleAggregator {
     ...
     * 
     * 🎯 ВАРИАНТ 3: МЯГКИЕ ГРАНИЦЫ для контролируемой "драмы"
     * - Без экстремальных значений (5% слишком мало, 15% слишком мало)
     * - Контролируемые размеры свечей (не в пол экрана)
     * - Плавные переходы между фазами
     */
    getPhasedVolatilityMultiplier(tickTime) {
        if (!this.usePhasedSystem || !this.currentCandle) {
            return 1.0; // Обычная волатильность для коротких таймфреймов (S5-M1)
        }
        
        const elapsed = tickTime - this.currentCandle.time;
        const progress = elapsed / this.timeframeSeconds;
        
        // 🎨 ФАЗЫ с МЯГКИМИ ГРАНИЦАМИ (имитация Pocket Option с контролем):
        // 0-10%: Birth - небольшая свеча (40% волатильности) ⬆️ было 15%
        // 10-75%: Growth - активный рост до полного размера (100% волатильности)
        // 75-92%: Stabilization - "рынок замедлился", умеренные движения (30% волатильности) ⬆️ было 5%
        // 92-100%: Finale - неожиданный всплеск (60% волатильности) ⬆️ было 45%
        
        if (progress < 0.10) {
            // 📍 BIRTH: Небольшая свеча (40% от нормальной волатильности)
            return 0.40;
        } else if (progress < 0.75) {
            // 📈 GROWTH: Плавный рост от 40% до 100%
            const growthProgress = (progress - 0.10) / 0.65;
            return 0.40 + (0.60 * growthProgress);
        } else if (progress < 0.92) {
            // 😴 STABILIZATION: "Рынок медленный", но не мертвый (30% волатильности)
            return 0.30;
        } else {
            // 💥 FINALE: Умеренный всплеск для непредсказуемости (60% волатильности)
            return 0.60;
        }
    }
    ...
    addTick(tick) {
        ...
        if (!this.currentCandle || this.currentCandle.time !== candleStartTime) {
            ...
            // 🎨 ФАЗОВАЯ ФИЛЬТРАЦИЯ для длинных таймфреймов
            if (this.usePhasedSystem) {
                const elapsed = tick.time - this.currentCandle.time;
                const progress = elapsed / this.timeframeSeconds;
                
                // Определяем вероятность принятия тика в зависимости от фазы
                let acceptanceProbability = 1.0;
                
                if (progress < 0.10) {
                    // Birth: 60% тиков (умеренная активность)
                    acceptanceProbability = 0.60;
                } else if (progress < 0.75) {
                    // Growth: 100% тиков (полная активность)
                    acceptanceProbability = 1.0;
                } else if (progress < 0.92) {
                    // Stabilization: 40% тиков (замедление)
                    acceptanceProbability = 0.40;
                } else {
                    // Finale: 80% тиков (финальный всплеск)
                    acceptanceProbability = 0.80;
                }
                
                // Случайная фильтрация тиков (кроме последнего тика свечи)
                if (Math.random() > acceptanceProbability) {
                    // Игнорируем этот тик, но ВСЕГДА обновляем close
                    // (чтобы цена не "застревала" на месте)
                    this.currentCandle.close = tick.price;
                    return { isNewCandle: false, currentCandle: { ...this.currentCandle } };
                }
            }
            
            // Обновляем текущую свечу (обычная логика или после прохождения фазовой фильтрации)
