...
class TickGenerator {
    ...
    async generateHistoricalData() {
        ...
        // 🔥 СОХРАНЯЕМ оригинальный Math.random() перед детерминированной генерацией
        ...
        for (let i = 0; i < totalTicks; i++) {
            // Генерируем изменение цены (Geometric Brownian Motion) с УВЕЛИЧЕННОЙ волатильностью
            price = this.generateNextPrice(price, true); // true = historical mode
        ...
        // 🔥 ВОССТАНАВЛИВАЕМ оригинальный Math.random() для реал-тайм генерации!
    ...
    generateNextPrice(currentPrice, isHistorical = false) {
        ...
        if (isHistorical) {
            ...
            // Используем 8% от волатильности за тик (было 2%)
            randomShock = this.gaussianRandom() * this.volatility * 0.08;
        } else {
