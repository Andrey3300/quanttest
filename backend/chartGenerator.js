...
const SYMBOL_CONFIG = {
    ...
    'TEST_TEST1': { basePrice: 1.0, volatility: 0.0002, type: 'CRYPTO' }, // 🎯 POCKET OPTION CALIBRATED: S5≈0.01%, M3≈0.05%, M30≈0.3%
...
class TickGenerator {
    ...
    generateNextPrice(currentPrice, isHistorical = false) {
        // 🎯 POCKET OPTION CALIBRATED: Генерация тиков для TEST/TEST1
        // Тики должны ТОЛЬКО обновлять текущую свечу, не создавать огромных движений
        // Целевая волатильность: S5≈0.01%, M3≈0.05% (как в реальном EUR/USD OTC)
        if (this.symbol === 'TEST_TEST1') {
            // 1. Плавный тренд (обновляем счётчик и направление)
            // КРИТИЧНО: Уменьшено в ~50-100 раз для масштабирования на тики (500ms интервал)
            this.trendCounter--;
            if (this.trendCounter <= 0) {
                ...
                this.trendStrengthTest = 0.000005 + Math.random() * 0.000015; // 0.000005-0.00002 (0.0005%-0.002% за тик)
            }
            
            // 2. Волатильность (микро-движения цены на каждый тик)
            // За 360 тиков в M3: 360 * 0.00002 ≈ 0.7% максимум (приемлемо)
            const volatility = 0.000008 + Math.random() * 0.000012; // 0.000008-0.00002 (0.0008%-0.002%)
            const noise = (Math.random() - 0.5) * volatility;
            
            // 3. Имитация рыночного пульса (микро-волны)
            const pulse = Math.sin(Date.now() / 3000) * 0.000003; // 0.0003% пульсация
            
            // 4. Обновляем цену (БЕЗ сильного meanReversion для естественного движения)
            let next = currentPrice + this.trendDir * this.trendStrengthTest + noise + pulse;
            
            // Мягкие ограничения (±5% от базовой цены - достаточно для естественного движения)
