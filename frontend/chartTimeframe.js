...
class ChartTimeframeManager {
     ...
     * - Для M3 (3 минуты): свечи начинаются в :00, :03, :06, :09... каждого часа
    ...
    getCandleStartTime(timestamp, timeframe) {
        ...
        if (duration < 60) {
            ...
            const periodSeconds = Math.floor(seconds / duration) * duration;
        ...
        const targetMinutes = duration / 60; // Длительность в минутах
